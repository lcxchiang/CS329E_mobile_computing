...
class SelectionViewController: UIViewController {
    ...
    @IBAction func donePressed(_ sender: Any) {
        ...
        if (crust == "") {
            ...
            let newPizza = Pizza(pizzaSize: pizzaSize, crust: crust, cheese: cheese, meat: meat, veggies: veggies)
            let otherVC = delegate as! AddPizza
            otherVC.addPizza(Pizza: newPizza)
            pizzaLabel.text = "One \(newPizza.pizzaSize) pizza with:\n\t\(newPizza.crust)\n\t\(newPizza.cheese)\n\t\(newPizza.meat)\n\t\(newPizza.veggies)"
        }
